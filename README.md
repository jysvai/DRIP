...
| `game/public/city/` | 시내 도로망 `seoul.json` (교차점 1만 3천여 곳·도로 토막 1만 8천여 개·신호 2,481곳·역·건물 이름 5,408곳, `pipeline/osm_city.py`) |
...
- **교차로:** 정지선·횡단보도·모서리 보도, 가로형 4색 신호등(적·황·좌회전 화살표·녹). 신호는 서울 간선도로에 흔한 동시신호(주도로 직진 → 주도로 좌회전 → 교차도로 직진 → 교차도로 좌회전, 황색 3초·전적색 2초)이고 교차로마다 시작 시각이 다르다 (`city/signals.ts`). 우회전은 적색이어도 정지선에서 멈춘 뒤 돌 수 있다.
- **내비:** 도는 교차로만 1km·300m·직전에 음성으로 알리고("300미터 앞, ○○교차로에서 올림픽로 방향으로 좌회전입니다"), 500m 앞부터 돌 수 있는 차로를 보여 준다. 오른쪽 목록은 다음 신호 교차로 셋, 미니맵은 둘레 시내 도로.
...
- **주변 차 (`city/traffic.ts`):** 경로 도로의 같은 방향·반대편 차는 고속도로와 같은 차로 모형(IDM·MOBIL, 한국식 습관·끼어들기)으로 달리고, 적색 신호에 정지선 앞에 줄을 선다. 교차로마다 일부는 경로를 벗어나 우회전·좌회전해 나가고, 옆 도로에서는 신호에 맞춰 건너오는 차가 나타나 경로 도로로 들어오거나 지나간다. 교차로 안에서는 이미 들어간 차가 먼저 가고(도로교통법 제26조), 길이 엇갈리면 만나는 곳에 먼저 닿는 차가 간다. 건너편 나갈 자리가 막혀 있으면 파란불에도 들어가지 않고(꼬리물기로 교차로가 막히지 않게), 적색 우회전은 멈춘 뒤 건너오는 차가 없을 때 돈다. 교통량은 메뉴의 교통 설정과 시간대를 따른다.
- **아직 없는 것:** 버스전용차로·보행자, 국도(지방 도로).
