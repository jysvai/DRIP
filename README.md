...
- **도로망:** OSM 도로 토막을 교차로(가까이 모인 교차점을 하나로)·한 방향 링크·이동(직진·좌회전·우회전)으로 바꾼다. 중앙분리대가 있어 두 줄로 그려진 큰길은 같은 이름·반대 방향 짝을 찾아 한 도로로 본다. 경로는 교차로 곡선으로 이어 한 줄 도로로 만들어 고속도로와 같은 물리·화면을 쓴다.
...
- **지형:** 지역 전체 75m 지형 격자(AWS Terrain Tiles)로 땅을 깔아 산·골짜기가 보이고, 도로는 40m마다 지형을 따라 오르내린다(다리·터널은 양 끝 사이를 곧게). 도로 옆은 둑·깎기로 지형에 이어지고, 상·하행 차도 높이가 다르면 가운데가 옹벽이 된다. 가까운 900m 너머는 먼 산 한 장(100m 격자)으로 2.8km까지 그린다.
