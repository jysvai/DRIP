...
- **판정 (주행 중에는 안 보여 줌):** 신호 위반, 적색 우회전 전 일시정지 안 함, 정지선 넘어 정지, 회전 방향지시등 없음·늦음(30m), 회전할 수 없는 차로에서 회전, 중앙선 침범. 교차로마다 `junction_pass`(신호 색·회전·차로)를, 황색이 켜진 순간 거리·속도와 통과·정지를 `signal_amber`로 남긴다 (딜레마 구간 분석용).
- **아직 없는 것:** 주변 차(다음 단계), 버스전용차로·보행자, 국도(지방 도로).
